...
cl_shared_set_combo_tbl<-inter_set_tbl %>% 
  mutate(su=HMEC+H1+GM12878) %>% 
  filter(su>=2) %>% 
  dplyr::select(GO.Set)

cl_exclusive_set_tbl<-inter_set_tbl %>% 
  mutate(su=HMEC+H1+GM12878) %>% 
  filter(su==1) %>% 
  dplyr::select(GO.Set)
...
shared_GO_set_vec<-unique(cl_shared_set_combo_tbl$GO.Set)

cell_line_shared_GO_full_tbl<-do.call(bind_rows,lapply(cell_line_files,function(file){
  tmp_line<-strsplit(file,split="_")[[1]][3]
  gene_set_enrich_tbl<-get(load(paste0("./data/",file)))
  tmp_obj<-names(mget(load(paste0("./data/",file))))
  rm(list=tmp_obj)
  rm(tmp_obj)
  return(gene_set_enrich_tbl %>% filter(Gene.Set%in%shared_GO_set_vec) %>% arrange(FDR) %>% mutate(line=tmp_line))
  
}))

do.call(bind_rows,lapply(unique(comm_edge_tbl$x),function(comm){
  tmp_set<-cell_line_shared_GO_full_tbl %>% 
...
tmp_set<-cell_line_shared_GO_full_tbl %>% 
  inner_join(.,cl_shared_set_combo_tbl%>%dplyr::select(GO.Set,GO.ID),by=c("Gene.Set" = "GO.Set")) %>% 
  inner_join(.,comm_edge_tbl %>% filter(x==3) %>% 
               summarise(GO.ID=unique(c(ego,alter))))
...
#----------------------
## Examine same properties but exclusive sets
cl_exclusive_set_tbl<-inter_set_tbl %>% 
  mutate(su=HMEC+H1+GM12878) %>% 
  filter(su==1) %>% 
  dplyr::select(GO.Set)
...
cl_exclusive_set_tbl<-cl_exclusive_set_tbl %>% 
  left_join(.,hm_gene_set%>%dplyr::select(V1,V2),by=c("GO.Set" = "V1")) %>% 
  mutate(GO.ID=map_chr(V2,function(x){
    unlist(get_GO_ID_fn(x))
  }))%>% 
...
simMatrix <- calculateSimMatrix(cl_exclusive_set_tbl$GO.ID,
                                orgdb="org.Hs.eg.db",
                                ont="BP",
                                method="Resnik")
d<-as.dist(1/(simMatrix+1e-3))
...
image(tmp_mat[get_order(order),get_order(order)],col=plasma(length(sample_comm)+1))
#----------------------------
exclu_GO_set_vec<-unique(cl_exclusive_set_tbl$GO.Set)

cell_line_exclu_GO_full_tbl<-do.call(bind_rows,lapply(cell_line_files,function(file){
  tmp_line<-strsplit(file,split="_")[[1]][3]
  gene_set_enrich_tbl<-get(load(paste0("./data/",file)))
  tmp_obj<-names(mget(load(paste0("./data/",file))))
  rm(list=tmp_obj)
  rm(tmp_obj)
  return(gene_set_enrich_tbl %>% filter(Gene.Set%in%exclu_GO_set_vec) %>% arrange(FDR) %>% mutate(line=tmp_line))
  
}))

do.call(bind_rows,lapply(unique(comm_edge_tbl$x),function(comm){
  tmp_set<-cell_line_exclu_GO_full_tbl %>% 
    inner_join(.,cl_exclusive_set_tbl%>%dplyr::select(GO.Set,GO.ID),by=c("Gene.Set" = "GO.Set")) %>% 
    inner_join(.,comm_edge_tbl %>% filter(x==comm) %>% 
                 summarise(GO.ID=unique(c(ego,alter))))
  return(tibble(set=comm,pvalue=kruskal.test(-log10(FDR) ~ line, data = tmp_set)$p.value))  
})) #%>% filter(pvalue<0.05)
