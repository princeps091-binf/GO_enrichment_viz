...
foreground_gene_file<-"./data/HMEC_5kb_top_compound_hub_ENSG_tbl.Rda"

out_file<-"./data/HMEC_5kb_tss_compound_hub_GOBP_enrich_tbl.Rda"
...
foreground_res_content_tbl<-foreground_gene_tbl %>% 
  group_by(res) %>% 
  summarise(ENSG=list(unique(unlist(ENSG.content))))

foreground_res_content_tbl<- foreground_res_content_tbl %>% 
  mutate(entrez=map(ENSG,function(x){
    gene_conv_tbl %>% 
...
path_tbl<-GO_set_enrich_fn(foreground_res_content_tbl$entrez[[4]],background_gene_vec,Gene_set_l)
print(path_tbl %>% 
        filter(FDR<=0.01) %>% 
        arrange(FDR),n=200)
